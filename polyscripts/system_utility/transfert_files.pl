...
foreach my $project_name (@$project_names) {
	...
	foreach my $pat (@$patients) {
		...
		#fastq
		my $dir_fastq = $pat->getSequencesDirectory();
			...
			eval {
				foreach my $fastq ( @{ $pat->fastqFiles } ) {
					foreach my $f ( values %$fastq ) {
						next unless ( $f =~ /_[RI][123].*\.fastq\.gz$/ );
						push( @$fastq_files, ( $f ) );
					}
				}
				if ($fastq_files) {
					print "\t" . join( "\n\t", @$fastq_files ) . "\n" if ($fastq_files);
					push( @$files, @$fastq_files ) if ($fastq_files);
				}
			};
			if ($@ or !$fastq_files) {
				colored::stabilo( "red", "No fastq file found" );
				die("No fastq file found for patient '$pat_name': $dir_fastq\n".$@) unless ($no_die);
				warn $@;
			}
		}
...
unless ($no_archive) {
	...
	if (-f $archive_dir.$archive_name) {
		system("ls -lh $archive_dir$archive_name");
		$prompt = prompt("An archive '$archive_name' already exixts in '". abs_path($archive_dir)."/'. Use this archive ?", -yes_no);
	...
	unless ($prompt) {
		...
		$question .= " ?  (y/n) ";
		die unless (prompt($question, -yes_no));
	
		my $cmd = "tar -cvf $archive_dir$archive_name " . join( " \\\n", @$files );
		$cmd =~ s/-cvf/-cvzf/ if ( grep { $_ =~ /htlv1/i } @file_types );
		print "\n";
		#warn $cmd;
		my $exit_tar = system $cmd;
		system("ls -lh $archive_dir$archive_name");
		confess('Error while making the archive.'."\n".$cmd) if ( $exit_tar or ! -e $archive_dir.$archive_name );
	}
...
if ($transfert_sftp and $no_archive ) {
	...
	my $i = 0;
		printf( "%d / %d\r", $i, scalar @$files );
	for my $file (@$files) {
		$sftp->put( "$archive_dir$archive_name", $file ) or confess("\nCould not transfer file '$file' on the sftp: " . $sftp->error );
		$i ++;
		printf( "%d / %d\r", $i, scalar @$files );
	}
	print "\n";
...
Optional arguments
	...
	-archive_dir <s>	path to create the archive before putting it on the sftp, default '/data-pure/workspace/download/transfert_file/'
...
# --- Fonction utilitaire pour formatage humain --- #
sub human_size {
    my ($bytes) = @_;
    return sprintf("%d B", $bytes)           if $bytes < 1024;
    return sprintf("%.2f KB", $bytes/1024)   if $bytes < 1024**2;
    return sprintf("%.2f MB", $bytes/1024**2) if $bytes < 1024**3;
    return sprintf("%.2f GB", $bytes/1024**3) if $bytes < 1024**4;
...
sub callback {
    ...
    # Progression depuis la dernière boucle
    my $delta_bytes = $offset - $last_offset;
    my $delta_time  = $now - $last_time || 1;

    # Débit moyen depuis le début
    my $elapsed = $now - $start_time;
    my $avg_speed = $offset / ($elapsed || 1);  # bytes/sec

    # ETA
    my $remaining_bytes = $size - $offset;
    my $eta = $remaining_bytes / ($avg_speed || 1);

    # Format humain
    my $pct = sprintf("%.1f", ($offset / $size) * 100);
    my $h_offset = human_size($offset);
    my $h_size   = human_size($size);
    my $h_speed  = sprintf("%.2f MB/s", $avg_speed / (1024*1024));

    my $eta_h = int($eta / 3600);
    my $eta_m = int(($eta % 3600) / 60);
    my $eta_s = int($eta % 60);

    printf(
        "Upload: %s%% - %s / %s - %s - ETA %02d:%02d:%02d\r",
        $pct, $h_offset, $h_size, $h_speed, $eta_h, $eta_m, $eta_s
    );
