...
#my $chromosomes = [22];
my $dir_rock = "/data-isilon/public-data/repository/HG19/prediction_matrix/$version/rocks/";
system("mkdir -p $dir_rock; chmod a+rwx $dir_rock") unless -e $dir_rock;
...
sub save_matrix {
	my ($chr) = @_;
my $nop = GenBoNoSqlRocksAnnotation->new(dir=>$dir_rock,factor=>$factor,pack=>$pack,mode=>"t",name=>$chr,version=>"110",pipeline=>1,description=>$description);	
	my $buffer = ensembl_buffer->new();
	 $buffer->getConfig->{ensembl}->{HG38} = $version;
...
foreach my $gene (@genes) {
    ...
    foreach my $transcript (@transcripts) {
        ...
        if ($protein) {
			...
			$nop->put_batch_raw($genbo_id,$md5);
			$nop->put_batch_raw($md5,$genbo_id);
        }
