...
die() unless -e $file;
	my $no = GenBoNoSqlRocksAnnotation->new(dir=>$dir_out,mode=>"t",name=>$chr,pack=>[],version=>"",description=>{},pipeline=>1);
	my $no_class = GenBoNoSqlRocksAnnotation->new(dir=>$dir_out,mode=>"t",name=>$chr.".class",pack=>[],version=>"",description=>{},pipeline=>1);
	my @objs;
		...
		while (my $x = $vcf->next_data_hash()) {
			...
			#	p $x if scalar(@$varAllele) >2 && $caf;
				...
				#	 warn $x->{INFO}->{CLNSIG} if $x->{INFO}->{CLNSIG} =~/\// ;
					...
					if ($vcfVarAllele eq "<DEL>"){
						$vcfVarAllele = "del". abs($x->{INFO}->{SVLEN});
					}
					if ($vcfVarAllele eq "<DUP>"){
						die($vcfVarAllele);
					}
					if ($vcfVarAllele =~ /</){
						die($vcfVarAllele);
					...
					my $rocks_id = $no->return_rocks_id($$x{'POS'},$$x{'REF'},$vcfVarAllele);
					$no->put($rocks_id,$variation);		
					$no_class->put_raw($rocks_id,$x->{INFO}->{CLASS});					
