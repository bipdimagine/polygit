...
confess ("You must be logged in under Cécile user ID: try 'su masson' before running the command line. Current user: $current_user") if ($current_user ne "masson");
my $sftp = Net::SFTP->new("192.168.2.111", user=>"bioinfo", key_path=>"/home/masson/.ssh/known_hosts");
...
my $dir_sftp = prompt("Select a directory to put the files: ", -menu=>\@file_names);
$sftp = undef;
...
$sftp->put("$archive_dir/$archive_name", "/$dir_sftp/$archive_name", callback()) or confess("Could not transfer the archive on the sftp");
