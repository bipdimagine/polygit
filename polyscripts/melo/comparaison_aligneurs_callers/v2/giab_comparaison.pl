...
@callers = grep { $_ ne 'melt' } @callers;
my $nb_callers = scalar @callers;

my $nb_alleles;
my $low_dp;

# Variants trouvés par les méthodes de calling
my $all_var = $test->getStructuralVariations;
#$nb_alleles->{'at least 1 caller'}->{'total'} = scalar @$all_var;
warn ("nb var all callers (including indels and low dp) for $test_patient_name: ".scalar @$all_var);
die ("No variant found for '$test_patient_name'. Check the vcf files and calling methods registered") unless (scalar @$all_var);

# Variants GIAB NIST (ref)
my $all_var_giab = $control->getStructuralVariations;
#$nb_alleles->{'nist giab'}->{'total'} = scalar @$all_var_giab;
warn "nb var giab (including indels) for $control_patient_name: ".scalar @$all_var_giab;
...
foreach my $var (@$all_var) {
	...
	$nb_alleles->{'at least 1 caller'}->{'SNP'} ++ if ($var->isVariation);
	$nb_alleles->{'at least 1 caller'}->{'indel'} ++ if ($var->isInsertion or $var->isDeletion);
	$nb_alleles->{'at least 1 caller'}->{'SV'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
	$nb_alleles->{'at least 1 caller'}->{'total'} ++;
	if ($var->isVariation) {
		$nb_alleles->{'at least 1 caller'}->{'TP'} ++ if (exists $annex->{$control->id});
		$nb_alleles->{'at least 1 caller'}->{'FP'} ++ unless (exists $annex->{$control->id});
	}
	
	# Variants trouvés par chacune des méthode de calling
	my @callers_var = keys %{$hvar->{method_calling}};
	foreach my $caller (@callers_var) {
			$nb_alleles->{$caller}->{'total'} ++;
			$nb_alleles->{$caller}->{'SNP'} ++ if ($var->isVariation);
			$nb_alleles->{$caller}->{'indel'} ++ if ($var->isInsertion or $var->isDeletion);
			$nb_alleles->{$caller}->{'SV'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
			if ($var->isVariation) {
				$nb_alleles->{$caller}->{'TP'} ++ if (exists $annex->{$control->id});
				$nb_alleles->{$caller}->{'FP'} ++ unless (exists $annex->{$control->id});
				push(@{$lists->{'P'}->{$caller}}, $var->id);
				push(@{$lists->{'TP'}->{$caller}}, $var->id) if (exists $var->annex->{$control->id});
				push(@{$lists->{'FP'}->{$caller}}, $var->id) unless (exists $var->annex->{$control->id});
			}
	}
	
	# Variants trouvés par toutes les méthodes de calling
	my $nb_callers_var = scalar grep ( '/('.join('|',@callers).')/', @callers_var);
	if ($nb_callers_var == $nb_callers) {
			$nb_alleles->{"all $nb_callers callers"}->{'total'} ++;
			$nb_alleles->{"all $nb_callers callers"}->{'SNP'} ++ if ($var->isVariation);
			$nb_alleles->{"all $nb_callers callers"}->{'indel'} ++ if ($var->isInsertion or $var->isDeletion);
			$nb_alleles->{"all $nb_callers callers"}->{'SV'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
			if ($var->isVariation) {
				$nb_alleles->{"all $nb_callers callers"}->{'TP'} ++ if (exists $annex->{$control->id});
				$nb_alleles->{"all $nb_callers callers"}->{'FP'} ++ unless (exists $annex->{$control->id});
			}
	}
	
	foreach my $i (0 .. $#callers) {
		$Venn->{'P'}->{$var_id} |= (1 << $i) if ($var->isVariation && exists $hvar->{'method_calling'}->{$callers[$i]} );#&& exists $annex->{$control->id});
...
foreach my $var (@$all_var_giab) {
	...
	$nb_alleles->{'nist giab'}->{'SNP'} ++ if ($var->isVariation);
	$nb_alleles->{'nist giab'}->{'indel'} ++ if ($var->isInsertion or $var->isDeletion);
	$nb_alleles->{'nist giab'}->{'SV'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
	$nb_alleles->{'nist giab'}->{'total'} ++;
	if ($var->isVariation) {
		$nb_alleles->{'at least 1 caller'}->{'FN'} ++ unless (exists $annex->{$test->id});
#		push(@{$lists->{'giab'}->{'giab'}}, $var->id))
		foreach my $caller (@callers) {
			$nb_alleles->{$caller}->{'FN'} ++ unless (exists $annex->{$test->id}->{'method_calling'}->{$caller});
			push(@{$lists->{'FN'}->{$caller}}, $var->id) unless ( exists $var->annex->{$test->id}->{'method_calling'}->{$caller})
		}
		my @callers_var = keys %{$var->annex->{$test->id}->{'method_calling'}};
		my $nb_callers_var = scalar grep ( '/('.join('|',@callers).')/', @callers_var);
		$nb_alleles->{"all $nb_callers callers"}->{'FN'} ++ unless ($nb_callers_var == $nb_callers);
		
		foreach my $i (0 .. $#callers) {
			$Venn->{'TP_giab'}->{$var_id} |= (1 << $i) if (exists $annex->{$test->id}->{'method_calling'}->{$callers[$i]} );
			$Venn->{'FN'}->{$var_id} |= (1 << $i) unless (exists $annex->{$test->id}->{'method_calling'}->{$callers[$i]} );
		}
	}
}
...
my @columns = ('SNP','indel','total','TP','FP','FN','Accuracy','Sensitivity');
print {$out} '#caller'."\t".join("\t", @columns) ."\n";
my @lines = @callers;
push(@lines, 'at least 1 caller', "all $nb_callers callers", 'nist giab');
foreach my $caller (@lines) {
	print {$out} $caller;
	unless ($caller eq 'nist giab' or !$nb_alleles->{$caller}->{'TP'} ) {
		$nb_alleles->{$caller}->{'Accuracy'} = $nb_alleles->{$caller}->{'TP'} /( $nb_alleles->{$caller}->{'TP'} + $nb_alleles->{$caller}->{'FP'} );
		$nb_alleles->{$caller}->{'Sensitivity'} = $nb_alleles->{$caller}->{'TP'} /( $nb_alleles->{$caller}->{'TP'} + $nb_alleles->{$caller}->{'FN'} );
	}
	foreach my $col (@columns) {
		print {$out} "\t".$nb_alleles->{$caller}->{$col};
	}
	print {$out} "\n";
}
close($out);
...
foreach my $cat ('P','TP','FP','FN') {
	system("mkdir $dir$cat") unless (-d $dir.$cat);
	foreach my $caller (@callers) {
		my $file_liste_var = $dir.$cat.'/variants_'.$caller.'.list';
#		warn $file_liste_var;
		open (my $fh, '>', $file_liste_var);
		print {$fh} join("\n", @{$lists->{$cat}->{$caller}})."\n" if (exists $lists->{$cat}->{$caller});
		close($fh);
	}
}
