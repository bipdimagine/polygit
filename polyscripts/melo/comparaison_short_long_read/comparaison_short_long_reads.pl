...
my $project = $buffer->newProjectCache(-name=>$project_name) || confess ("Can't open project '$project_name': $!");
warn $project->name;
...
my $align_meth = $control->alignmentMethods();
confess("More than one alignment method") if scalar(@$align_meth) > 1;
my $capture = $control->getCapture->name;
warn "Capture: ".$capture;
warn "Alignment: ".$align_meth->[0];
my $calling_meth = $control->alignmentMethods();
warn "Alignment: ".join(', ',@$calling_meth);

# tests
my $patients = $project->getPatients;
foreach my $pat (@$patients) {
	next unless ($pat->name eq $control_patient_name);
	warn 'Test patient: '.$pat->name."   id:".$pat->id;
	my $align_meth = $pat->alignmentMethods();
	confess("More than one alignment method") if scalar(@$align_meth) > 1;
	my $capture = $pat->getCapture->name;
	warn "Capture: ".$capture;
	warn "Alignment: ".$align_meth->[0];
	my $calling_meth = $pat->alignmentMethods();
	warn "Alignment: ".join(', ',@$calling_meth);
}
...
foreach my $chr (@{$project->getChromosomes}) {
	...
	foreach my $pat (@$patients) {
		...
		warn $pat_name;
		$vect->{$pat_name}->{'all variants'} = $pat->getVariantsVector($chr);
		$vect->{$pat_name}->{'snp'} = $pat->getVectorSubstitutions($chr);
		$vect->{$pat_name}->{'indel'} = $pat->getVectorIndels($chr);
	}
...
#	$nb_alleles->{'SV'}->{'at least 1 caller'}->{'total'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
#	$nb_alleles->{'all variants'}->{'at least 1 caller'}->{'total'} ++;
#	foreach my $type (@types) {
#		my $cond;
#		$cond = 1 if ($type eq 'all variants');
#		$cond = $var->isVariation if ($type eq 'snp');
#		$cond = $var->isInsertion or $var->isDeletion if ($type eq 'indel');
#		if ($cond) {
...
#		$nb_alleles->{'SV'}->{$caller}->{'total'} ++ if ($var->isLargeDeletion or $var->isLargeInsertion or $var->isLargeDuplication);
#		foreach my $type (@types) {
#			my $cond;
#			$cond = 1 if ($type eq 'all variants');
#			$cond = $var->isVariation if ($type eq 'snp');
#			$cond = $var->isInsertion or $var->isDeletion if ($type eq 'indel');
#			if ($cond) {
#				$nb_alleles->{$type}->{$caller}->{'TP'} ++ if (exists $annex->{$control->id});
#				push (@{$nb_alleles->{$type}->{$caller}->{'dp'}->{'TP'}}, $hvar->{'dp'}) if (exists $annex->{$control->id});
#				$nb_alleles->{$type}->{$caller}->{'FP'} ++ unless (exists $annex->{$control->id});
#				push (@{$nb_alleles->{$type}->{$caller}->{'dp'}->{'FP'}}, $hvar->{'dp'}) unless (exists $annex->{$control->id});
#				push(@{$lists->{$type}->{'P'}->{$caller}}, $var->id);
#				push(@{$lists->{$type}->{'TP'}->{$caller}}, $var->id) if (exists $var->annex->{$control->id});
#				push(@{$lists->{$type}->{'FP'}->{$caller}}, $var->id) unless (exists $var->annex->{$control->id});
#			}
#		}
#	}
#	
...
#		$nb_alleles->{'SV'}->{"all $nb_callers callers"} ++ if ($var->isLargeDeletion or $var->isLargeInsertion or $var->isLargeDuplication);
#		foreach my $type (@types) {
#			my $cond;
#			$cond = 1 if ($type eq 'all variants');
#			$cond = $var->isVariation if ($type eq 'snp');
...
#	$nb_alleles->{'SV'}->{'nist giab'}->{'total'} ++ if ($var->isLargeDeletion or $var->isLargeDeletion or $var->isLargeDuplication);
...
#		$cond = $var->isVariation if ($type eq 'snp');
#		$cond = $var->isInsertion or $var->isDeletion if ($type eq 'indel');
#		if ($cond) { # && not exists $annex->{$test->id} && not exists $annex->{$test->id}->{'method_calling'}) {
#			$nb_alleles->{$type}->{'at least 1 caller'}->{'FN'} ++ unless (exists $annex->{$test->id} && exists $annex->{$test->id}->{'method_calling'} && scalar $annex->{$test->id}->{'method_calling'});
#			warn Dumper $annex->{$test->id} if (exists $annex->{$test->id});
#	#		push(@{$lists->{$type}->{'giab'}->{'giab'}}, $var->id))
#			foreach my $caller (@callers) {
#				unless (exists $annex->{$test->id} && exists $annex->{$test->id}->{'method_calling'} && exists $annex->{$test->id}->{'method_calling'}->{$caller}) {
#					$nb_alleles->{$type}->{$caller}->{'FN'} ++ ;
#					push (@{$nb_alleles->{$type}->{$caller}->{'dp'}->{'FN'}}, $hvar->{'dp'});
#					push(@{$lists->{$type}->{'FN'}->{$caller}}, $var->id);
#				}
#			}
#			unless (exists $annex->{$test->id} && exists $annex->{$test->id}->{'method_calling'} && scalar $annex->{$test->id}->{'method_calling'}) {
#				my @callers_var = keys %{$var->annex->{$test->id}->{'method_calling'}};
#				my $nb_callers_var = scalar grep ( '/('.join('|',@callers).')/', @callers_var);
#				$nb_alleles->{$type}->{"all $nb_callers callers"}->{'FN'} ++ unless ($nb_callers_var == $nb_callers);
#			}
#			
##			foreach my $i (0 .. $#callers) {
