...
-------------------------------------------------------------------------------
   ...
   Optionally, one of these  (only -E and -T allowed with --dfam_infile)
    ...
    --species <i>        Use a species-specific library instead of the full
                         Dfam database. This is only available for species
                         that have had species-specific thresholds calculated,
                         and requires the file "species_name_dfam.hmm" to be
                         present in the same directory as the primary 'hmmfile'.
