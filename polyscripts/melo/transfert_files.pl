...
warn grep {$_ =~ /htlv1/i} @$file_types;
...
if ($transfert_sftp) {
	...
	$sftp->put("$archive_dir/$archive_name", "/$dir_sftp/$archive_name", \&callback) or confess("Could not transfer the archive on the sftp: ".$sftp->error);
	print "\n";
