...
sub getProjectLists {
	...
	if ($project_query){
	 foreach my $project ( @$res) {
		...
		$project->{pedigree} = 0;
		$project->{somatic} = 0;
		$project->{pedigree} = 1 if -e $buffer->getDataDirectory("root")."/".$project->{type}."/".$project->{name}."/".$project->{name}.".ped";
		$project->{somatic} = 1 if -e $buffer->getDataDirectory("root")."/".$project->{type}."/".$project->{name}."/".$project->{name}.".somatic";
