...
encode_json_table_versions($buffer);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_versions {
	my $res = $buffer->getQueryPolypipeline()->getVersionsList();
	export_data::print_simpleJson($cgi,$res, "version_id");
}
