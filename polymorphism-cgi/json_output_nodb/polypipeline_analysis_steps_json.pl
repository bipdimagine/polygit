...
warn $analysis_id ;
encode_json_table_analysis_steps($buffer,$analysis_id);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_analysis_steps {
	my ($buffer, $analysis_id) = @_ ;
	warn $analysis_id ;
	my $res = $buffer->getQueryPolypipeline()->getAnalysisSteps($analysis_id);
