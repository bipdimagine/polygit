...
#warn $analysis_id ;
encode_json_table_analysis_versions($buffer,$analysis_id);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_analysis_versions {
	my ($buffer, $analysis_id) = @_ ;
	warn $analysis_id ;
	my $res = $buffer->getQueryPolypipeline()->getAnalysisVersions($analysis_id);
