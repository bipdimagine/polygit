...
encode_json_table_programs($buffer);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_programs {
	my $res = $buffer->getQueryPolypipeline()->getProgramsList();
	export_data::print_simpleJson($cgi,$res, "program_id");
}
