...
#warn $analysis_id ;
encode_json_table_analysis_programs($buffer,$analysis_id);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_analysis_programs {
	my ($buffer, $analysis_id) = @_ ;
	warn $analysis_id ;
	my $res = $buffer->getQueryPolypipeline()->getAnalysisPrograms($analysis_id);
