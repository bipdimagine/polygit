...
#warn $analysis_id ;
encode_json_table_analysis_references($buffer,$analysis_id);


##fonction pour encoder en json les données des tables de la base

sub encode_json_table_analysis_references {
	my ($buffer, $analysis_id) = @_ ;
	warn $analysis_id ;
	my $res = $buffer->getQueryPolypipeline()->getAnalysisReferences($analysis_id);
	export_data::print_simpleJson($cgi,$res, "analysis_id");
}
