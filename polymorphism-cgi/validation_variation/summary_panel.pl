...
#@title = ("Fam","Patient","Sex","mendelian error","Cov","30x","Sub","Indels","%He","validation");
	...
	$line->{Patient} = $cgi->td( $class,
qq{<button type="button" class ="btn btn-xs btn-primary "  $style_btn_name >$ir</button>}
	);
	...
	$btn_class = qq{class= "btn  btn-xs btn-warning " style = "$fsize" }
	  if $v < 15;
	$btn_class = qq{class= "btn  btn-xs btn-alert " style = "$fsize" }
	  if $v < 10;
	$line->{Cov} =
	...
	if ( exists $hstatus->{term} && $hstatus->{term} eq "negative" ) {
		...
		my $text =
qq{ <span  class="stamp1"><span>-$term-</span>&nbsp;-&nbsp;<small>$date2</small>&nbsp-&nbsp;$u</span>};
		...
		if (@text) {
			$line->{"validation"} =
			  $cgi->td( { style => "background-color:#E74C3C;padding:3px" },
				join( "<BR>", @text ) );
		}
...
sub header_run {
	...
	$version->{gnomad} = $project->getChromosome('1')->rocksdb_version('gnomad');
	$version->{hgmd} = $project->getChromosome('1')->rocksdb_version('hgmd');
	$version->{cadd} = $project->getChromosome('1')->rocksdb_version('cadd');
	$version->{clinvar} = $project->getChromosome('1')->rocksdb_version('clinvar');
	...
	my $text_info = "";
	$text_info .=
qq{SOMATIC<span class="glyphicon glyphicon-minus" aria-hidden="true"></span> }
	  if $project->isSomatic;
	$text_info .= join(
		qq{<span class="glyphicon glyphicon-minus" aria-hidden="true"></span>},
		@$info );
	$text_info .=
qq{<span class="glyphicon glyphicon-minus" aria-hidden="true"></span>SOMATIC }
	  if $project->isSomatic;
...
# 		my $capture  = qq{<span class="label label-info " style="font-size: 12px;">}. $capt->name."</span>" ;
	...
	foreach my $name ( keys %$version ) {
		my $v = $version->{$name};
		$texta .=
qq{<span class="label " style="font-size: 12px;$bcolor">$name <span class="badge badge-alert" style="font-size: 10px;">$v </span></span>	};
	}
		...
		$out .= $cgi->start_table(
			{
				...
				style =>
"color:black;text-align: center;vertical-align:middle;font-size: 10px;font-family:  Verdana;box-shadow: 1px 1px 2px 1px YYY;margin-top: 5px;margin-bottom: 0px;"
			}
		);
...
"text-align: center;vertical-align:middle;font-size: 10px;font-family:  Verdana;box-shadow: 1px 1px 2px 1px YYY ;",
		...
		my $img =
		  qq{<img src="https://img.icons8.com/offices/24/000000/family.png">};
		$out .= qq{
...
sub parent_box {
	...
	unless ($p) {
		my $icon =
qq{<div class ="$class"> <img src="https://img.icons8.com/ios/32/000000/decision.png"> </div>};
		$out .= qq{
...
sub get_mendelian_statistics {
	...
	foreach my $line ( @{ $data->{data} } ) {
		...
		if ( $patient->isParent ) {
			...
			$p = $format_number->round(
				$p / scalar( @{ $patient->getFamily->getChildren } ), 1 )
			  if $p;
		}
...
sub statistics_projects {
	my ($run)   = @_;
	my $capture = $project->getCaptures()->[0];
	my $query   = $project->buffer->getQuery->listAllProjectsNameByCaptureId(
		$capture->id() );
				...
				foreach my $p_value ( @{ $data->{data} } ) {
					...
					foreach my $k ( keys %$p_value ) {
						...
						if ( $p eq $project->name ) {
							...
							push(
								@{
									$this_stats->{$k}->{ $patient->getRun->id }
								},
								$p_value->{$k}->{text}
							);
						}
...
qq{<p class="$p1"><i class="fa fa-circle" style="color:$color;margin-right: 5px;margin-left: 2px; "></i>$v <span> $value <span> <span><i>($lmean)</i> </span>};
	...
	my $btn = " btn-primary";
	my $text =
qq{ <img src="https://img.icons8.com/flat_round/20/000000/warranty-card.png"> Quality Control};
	my $color = "#00A591";
	if ( $error == 1 ) {
		$color = "#FFD662";
	}
...
qq{  <img src="https://img.icons8.com/flat_round/20/000000/warranty-card.png"> Quality Control };
	...
	my $run_id = $run->id;
	$out2 =
qq{<div class="btn  btn-info btn-xs " style="position:relative;bottom:1px;min-width:200px;;border-color:black;background-color:$color" onClick='collapse_panel("control_quality","$list_control_panels","$run_id")'>  $text <span class="badge badge-danger">$error</span> </div>};
	return ( $out2, $out );
...
sub construct_identito_vigilence {
	...
	my @iv_vcf;
	@iv_vcf = split( "", $p->identity_vigilance_vcf() )
	  if $p->identity_vigilance_vcf();
	my $level;
	unless (@iv_vcf) {
		for ( my $i = 0 ; $i < @iv ; $i++ ) {
			$iv_vcf[$i] = "";
	...
	my $url = "https://defidiag.polyweb.fr/cgi-bin/HG38/polymorphism-cgi/validation_variation/explain_identito_error.pl?project=".$project->name."&patient=".$p->name;
...
#warn abs(time -$t);
	...
	my $btn = " btn-primary";
	my $text =
qq{ <img src="https://img.icons8.com/material/14/000000/mars-symbol.png">/<img src="https://img.icons8.com/material/14/000000/venus-symbol.png"> Gender control };
	if ( $error > 0 ) {
		$text =
qq{  <img src="https://img.icons8.com/material/14/000000/mars-symbol.png">/<img src="https://img.icons8.com/material/14/000000/venus-symbol.png">Gender control };
		$btn = " btn-danger";
...
sub table_control {
	...
	unless (@$controls) {
		...
		my $out1 =
qq{<div  type ="button"  style="position:relative;bottom:1px;min-width:200px;border-color:black;display: none" class="btn  btn-xs"  disabled> <img src="https://img.icons8.com/ios/20/000000/empty-test-tube.png"> Control (Blanc) &nbsp;&nbsp</div>};
	...
	if (@$controls) {
		...
		foreach my $c (@$controls) {
			...
			foreach my $patient ( @{ $project->getPatients } ) {
				...
				my $res =
` $samtools idxstats $bam | awk '{sum+=\$4;sum2+=\$3} END {print sum";"sum2}'`;
				chomp($res);
				my @z = split( ";", $res );
				$reads += ( $z[0] + $z[1] );
...
` $samtools idxstats $bam | awk '{sum+=\$4;sum2+=\$3} END {print sum";"sum2}'`;
		...
		my $run_id = $run->id;
		$out1 =
qq{<div class="btn  btn-info btn-xs btn-$style" style="position:relative;bottom:1px;min-width:200px;border-color:black" onClick='collapse_panel("control_panel","$list_control_panels","$run_id")'> <img src="https://img.icons8.com/ios/20/000000/empty-test-tube.png"></span>Control (Blanc)  &nbsp;&nbsp;<span class="badge badge-info">$nb</span></div>};
		my $out;
...
"text-align: center;vertical-align:middle;font-size: 10px;font-family:  Verdana;",
		...
		$out .=
		  $cgi->th( { style => "text-align: center;" }, "Nb Reads Control" );
		$out .=
		  $cgi->th( { style => "text-align: center;" }, "Nb Reads Project" );
		$out .= $cgi->th( { style => "text-align: center;" }, "Ratio %" );
		$out .=
		  $cgi->th( { style => "text-align: center;" }, "Coverage Control" );
		$out .=
		  $cgi->th( { style => "text-align: center;" }, "Coverage Project" );
		$out .= $cgi->th( { style => "text-align: center;" }, "Ratio %" );
		$out .= $cgi->th( { style => "text-align: center;" }, "15x" );
		$out .= $cgi->th( { style => "text-align: center;" }, "30x" );
		$out .=
		  $cgi->th( { style => "text-align: center;" }, "% aligned reads" );
		$out .= $cgi->end_Tr();
		$out .= $out_table;
		$out .= $cgi->end_table();
		$out .= $cgi->end_div();
		...
		$out_table .= $cgi->td(
			{
				colspan => 16,
				style =>
"fborder-top-width: 0px;border-left-width: 0px;padding-top: 0px;padding-bottom: 0px;"
			},
...
sub table_duplicate {
	...
	unless ( $project->bundle_transcripts() ) {
		my $out1 =
qq{<div class="btn  btn-xs btn-dark "  style="position:relative;bottom:1px;min-width:200px;;border-color:black" disabled>  Regions Dups  &nbsp;&nbsp;<span class="badge badge-dark">0</span></div>};
		return ( $out1, "" );
	}
...
"text-align: center;vertical-align:middle;font-size: 10px;font-family:  Verdana;",
	...
	$out .= $cgi->th( { colspan => "5", style => "text-align: center;" },
		"Region Dups" );
	$out .= $cgi->end_Tr();
	...
	$out .= $cgi->start_table(
		{
			...
			style =>
"vertical-align:middle;text-align: left;font-size: 8px;font-family:  Verdana;"
		}
	);
	my $capture = $project->getCaptures()->[0];
	my $ver1    = $project->genome_version;
	...
	$div    .= $cgi->h4(
			join( "-", return_date( $project->creation_date ) )
		  . "&nbsp;"
		  . $machine
		  . "&nbsp;"
		  . $capture->name );
	...
	$out .= $cgi->end_Tr();
	my @infos = (
		"Familly",    "Patient",     "Sex", "SRY",
		"Status",     "Cov",         "30X", "Analyse",
		"validation", "nb variants", "user"
	);
	$out .= $cgi->start_Tr();
	  ...
	  my $patient ( sort { $a->name cmp $b->name } @{ $project->getPatients } )
	{
		...
		$out .=
		  $cgi->td( { style => "border: 1px solid black;" }, $patient->name );
		...
		$out .=
		  $cgi->td( { style => "border: 1px solid black;" },, $hsex->{$sex} );
		$out .= $cgi->td( { style => "border: 1px solid black;" },
			, $hsex->{$sex_eval} . "<small>(" . $cov_sry . ")</small>" );
		...
		if ( $patient->isMother ) {
			$out .= $cgi->td(
				...
				,
qq{<i class="fa fa-female fa-1x" aria-hidden="true" style="color:pink"></i>}
			);
		}
			...
			$out .= $cgi->td(
				{ style => "border: 1px solid black;" },
				,
qq{<i class="fa fa-male fa-1x" aria-hidden="true" style="color:blue"></i>}
			);
		}
		else {
			$out .= $cgi->td(
				{ style => "border: 1px solid black;" },
				,
qq{<i class="fa fa-child fa-1x" aria-hidden="true" style="color:red"></i>}
			);
		...
		if ( keys %$hval ) {
			...
			$out .=
			  $cgi->td( { style => "border: 1px solid black;" }, $st_date );
...
sub merge_range {
	...
	push( @$array,
		{ row => $row, col => $col, merge => $merge, value => $value } );
}
...
sub printPatientLineXls {
	...
	foreach my $k ( keys %{$hval} ) {
		...
		merge_range( $line_patient, $row, $col, $nb_merge, $patient->name );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
			$term . " - $pn - $st_date" );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge, $v->name );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
			$v->getChromosome()->name . ":" . $v->start . "-" . $v->end );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
			join( ";", @{ $hvariant->{value}->{ngs} } ) );
		$col++;
		##############
		####GNOMAD
		##############
		...
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
			$hvariant->{value}->{max_pop} );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
			$hvariant->{value}->{min_pop} );
		$col++;
		##############
		####DEJAVU
		##############
		merge_range( $line_patient, $row, $col, $nb_merge,
				$hvariant->{value}->{other_project} . "::"
			  . $hvariant->{value}->{other_patients} . "::"
			  . $hvariant->{value}->{other_patients_ho} );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
				$hvariant->{value}->{similar_projects} . "::"
			  . $hvariant->{value}->{similar_patients} . "::"
			  . $hvariant->{value}->{similar_patients_ho} );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,,
			$hvariant->{value}->{this_run_patients} );
		$col++;
...
#			 		$worksheet->merge_range($row,$col,$row+$nb_merge,$col,$v->gnomadAC,$bold_merge);$col++;
		...
		update_variant_editor::check_is_hgmd_dm_for_gene(
			$patient->getProject(),
			$hvariant, $gene );
		update_variant_editor::check_is_clinvar_pathogenic_for_gene(
			$patient->getProject(),
			$hvariant, $gene );
		$hvariant->{value}->{hgmd} = "-"
		  if $hvariant->{value}->{hgmd} =~ /minus/;
		$hvariant->{value}->{clinvar} = "-"
		  if $hvariant->{value}->{clinvar} =~ /minus/;
		merge_range( $line_patient, $row, $col, $nb_merge,
			$hvariant->{value}->{hgmd} );
		$col++;
		merge_range( $line_patient, $row, $col, $nb_merge,
...
#$worksheet->merge_range($row,$col,$row+$nb_merge,$col,$gene->external_name,$bold_merge);$col++;
		...
		foreach my $t (@$htranscripts) {
			...
			foreach my $v1 (@header_transcripts) {
				merge_range( $line_patient, $row, $vcol++, 0,
					$t->{value}->{$v1} );
			}
			$row++;
...
sub printWorkSheet {
	...
	return unless %$hval;
	my $line_patient;
	my $bold_merge = $workbook->add_format(
		valign => 'vcentre',
		align  => 'centre',
	);
	...
	foreach my $k ( keys %{$hval} ) {
		...
		#warn $nb_merge;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $patient->name, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $v->name, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $v->getChromosome()->name . ":" . $v->start . "-" . $v->end,
			$bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, join( ";", @{ $hvariant->{value}->{ngs} } ), $bold_merge );
		$col++;
		##############
		####GNOMAD
		##############
		update_variant_editor::vgnomad( $v, $hvariant );
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{ac}, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{an}, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{ac_ho}, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{max_pop}, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{min_pop}, $bold_merge );
		$col++;
		...
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{this_run_patients}, $bold_merge );
		$col++;
...
#			 		$worksheet->merge_range($row,$col,$row+$nb_merge,$col,$v->gnomadAC,$bold_merge);$col++;
		...
		#####################
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{hgmd}, $bold_merge );
		$col++;
		$worksheet->merge_range( $row, $col, $row + $nb_merge,
			$col, $hvariant->{value}->{clinvar}, $bold_merge );
		$col++;
...
sub table_patients_xls2 {
	...
	foreach my $c (@colors) {
		$bg_color->{$c} = $workbook->add_format(
			valign => 'vcentre',
			align  => 'centre',
	...
	$bg_color->{strike} = $workbook->add_format(
		valign => 'vcentre',
		align  => 'centre',
	...
	$out .= $cgi->start_table(
		{
			...
			style =>
"text-align: center;vertical-align:middle;font-size: 9px;font-family:  Verdana;padding:10px",
			'data-click-to-select' => "true",
			'data-toggle'          => "table"
		}
	);
...
sub list_genes {
	my ($project) = @_;
	map { $list_genes->{ $_->getGene->id }++ }
	  @{ $project->getListGenBoTranscripts };
	return $list_genes;
...
#$out.= $cgi->th({colspan=>7},"CNV");
	...
	foreach my $k ( keys %{$hval} ) {
		...
		my $btn_class =
qq{class= "btn btn-xs  btn-primary" style="background-color: $color;$fsize"};
		my $st_date = join( "-", return_date( $val->{modification_date} ) );
		my $term    = $buffer->getValidationTerm( $val->{validation} );

		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$term</button>} );
		my $pn = $val->{user_name};
		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$pn</button>} );
		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$st_date</button>} );
		my ( $type, $chr, $start, $end ) = split( "_", $k );
		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$type</button>} );
		$out .= $cgi->td( $class,
			qq{<button type="button"  $btn_class >$chr</button>} );
		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$start</button>} );
		$out .= $cgi->td( $class,
			qq{<button type="button" $btn_class >$end</button>} );
...
sub validation_table_new {
	my ( $p, $hval ) = @_;
	print "*";
	my @headers_validations =
	  ( "igv_web", "var_name", "trio", "gene", "table_transcript", );
	...
	$out .= $cgi->start_table(
		{
			class => "table table-striped table-bordered table-hover",
			style =>
"text-align: center;vertical-align:middle;font-size: 9px;font-family:  Verdana;margin-bottom: 0px;padding:2px",
			'data-click-to-select' => "true",
			'data-toggle'          => "table"
		}
	);
...
qq{class= "btn btn-xs  btn-primary" style="background-color: $color;$fsize"};
		...
		$hvariation->{gene} =
qq{<button type="button" $btn_class onClick="update_grid_gene_phenotypes('ENSG00000110367_11')">$gn</button>};
		update::trio( $project, undef, $hvariation, $p, $cgi, undef );
		$hvariation->{table_transcript} =
		  update::construct_table_transcript( $v, $cgi, \@header_transcripts,
			3, $gene, 1 );
...
sub args_quality {
	...
	if ( $project->existsnoSqlQuality ) {
		my $t =
		  stat( $project->quality_dir . "/" . $project->name . ".lite" )->[9];
