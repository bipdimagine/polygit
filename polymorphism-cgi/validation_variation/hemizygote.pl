...
my $project = $buffer->newProjectCache(
 ...
 my $cache_id = $patient_name;
 my	$no_cache    = GenBoNoSqlLmdbCache->new(
		dir     => $project->getCacheDir(),
		mode    => "w",
		name    => $patient_name.".hemi.cache",
		is_compress => 1,
	);
...
$text ="" if $dev;
if($text){
	 	my $cache_icon = qq{<span class="glyphicon glyphicon-floppy-saved" aria-hidden="true" style="text-align:right;font-size:10px;color:green"></span>};
		print $text;
...
$cgi->param(
    -name  => 'annot',
    -values => ["splicing","essential_splicing","nonsynonymous","stop","phase","maturemirna","frameshift","non-frameshift","predicted_splice_site"],
);
# $cgi->param(
#    -name  => 'patients',
#    -value => '6120GM007277',
...
my  $cache_icon = qq{<span class="glyphicon glyphicon-floppy-remove" aria-hidden="true" style="text-align:right;font-size:10px;color:red"></span>};
...
html::print_cgi_header($cgi);
print $cgi -> start_html(-title=>"toto",
						-style=>{
...
#							 -script=>{
#							 	-src=>  ["https://www.polyweb.fr/polyweb/js/polyjs/igv_methods.js", "https://www.polyweb.fr/polyweb/js/polyjs/polyweb.js"],
#                             	
#                             },
                            );
...
#warn "genes fin:".scalar(@$genes);
	...
	print "<table style='position: absolute;left: 25%;'><td>";
	print qq{<img src="https://img.icons8.com/material/96/000000/no-data-availible.png" width="200" height="200" /></td>};	
	print qq{<td><div class="knockout">No Variation Found</div></td></table>};
}
}
...
#	$pm->wait_all_children();
	...
	while ( my @tmp = $iter->() ) {
		...
		( $res->{genes}, $res->{total_time} ) = refine_heterozygote_composite_score_one( $project, \@tmp,$vid );
  ...
  #warn $chr->name;
   ...
   warn "2" if 	$debug && $hashVector->{$chr->name}->contains($testid) && $debug;
	...
	#				   die($chr->name) if $debug && $hashVector->{ $chr->name }->contains(7710) && $debug;
					if ( $tr eq "both" ) {
						$vtr |= $patient->getFamily()->getVectorMotherTransmission( $chr, $patient ) if $patient->getFamily->getMother;
						$vtr |= $patient->getFamily()->getVectorFatherTransmission( $chr, $patient ) if $patient->getFamily->getFather();
				...
				#$vtr |= $patient->getFamily()->getVector_family_dominant($chr);
					if ( $tr eq "strict_denovo" ) {
						...
						#my $vtr2 = $vtr->Clone();
						my $vtr2 = $patient->getFamily()->getVectorMotherTransmission( $chr, $patient ) if $patient->getFamily->getMother;
