...
if ($project->isDiagnostic() or $gene_id) {}
else { exit(0); }

#exit(0) if $patients->[0]->isNoSqlDepth;
#exit(0);
print qq{ \{"status":"};
my $out;
...
if ($project->isDiagnostic()) {
	preload_coverage::load_coverage_regions_dup($project,$patients,\@transcripts,1);
	print "end list ";
	print qq{OK"\}} ;
	exit(0);
}
