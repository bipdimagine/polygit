...
my $VERSION_UPDATE_VARIANT_EDITOR =
  "9dd1dc46f32240dad92ea588d85d9f9d1";    #23/09/2021;
my $dev;
...
my @header_transcripts_cnv =
  ( "consequence", "enst", "nm", "ccds", "appris", "start", "end" );
...
unless ( $cgi->param('phenotype') ) {
	if ( $project->phenotypes ) {
		$cgi->param(
			-name  => 'phenotype',
			-value => $project->phenotypes->[0]
		);
	}
}
if ($gene_name_filtering) {
	my $gene;
	eval { $gene = $project->newGene($gene_name_filtering); };
	if ($gene) {
		...
		error(  " ERROR GENE NAME <HR> WHAT DO YOU MEAN BY : \""
			  . $gene_name_filtering
			  . "\"" );
		exit(0);
	}
...
#$dev = $cgi->param( 'force' ) == 1 ;
#$cgi->param(-name=>'force',-value=>0);
my $no_cache;
...
#	die();
#	$no_cache = $patient->get_lmdb_cache("c");
#}
#else {
...
if ( $project->isDiagnostic() ) { $level_dude = 'high,medium,low'; }
...
warn "no cache ******************** " . $project->name . " " . $patient->name;
my $version;
...
my ( $vectors, $list, $hash_variants_DM ) =
  constructChromosomeVectorsPolyDiagFork( $project, $patient, $statistics );
warn " 1 Vector---------> " . ( abs( time - $t ) );
...
foreach my $g ( sort { $b->{max_score} <=> $a->{max_score} } @$genes ) {
	if ($gene_name_filtering) {
		if (   $gene_name_filtering eq $g->{name}
			or $gene_id_filtering eq $g->{id} )
		{
			$hchr->{ $g->{chr_name} }++;
			push( @$vgenes, $g );
		}
...
my $stdout_nav_bar = tee_stdout {
	...
	update_variant_editor::printNavBar( $patient, $genes, $statistics,
		$version, $date, $user, $ztime,
		update_variant_editor::compose_string_filtering($cgi) );
};
my $stdoutcnv;
...
my $stdout_end = tee_stdout {
	...
	if (@$genes) {
		my $x = time;
		$genes =
		  refine_heterozygote_composite_score_fork( $project, $genes, $hchr );
		warn " ---------> " . ( abs( time - $x ) );
		print ' 2-> ' . abs( time - $x );
		$ztime .= ' 2-> ' . '_genes:' . ( abs( time - $x ) );
		warn $ztime;
		...
		if ($gene_name_filtering) {
			print
qq{<div class="knockout">No Variation Found for gene $gene_name_filtering</div>};
		}
		else {
...
sub calculate_max_score {
	...
	foreach my $hgene (@$list) {
		...
		$hgene->{chr_name} = my $chr = $gene->getChromosome();
		...
		foreach my $k ( keys %{ $hgene->{all_variants} } ) {
			if ( exists $h_all_variants_validations->{ $gid . '!' . $k } ) {
				my $score_validation =
				  $h_all_variants_validations->{ $gid . '!' . $k }->[0]
				  ->{validation};
			...
			push(
				@{ $class->{biallelic} },
				$hgene->{all_variants}->{$k}->{score}
			) if exists $hgene->{all_variants}->{$k}->{biallelic};
			push( @{ $class->{mother} }, $hgene->{all_variants}->{$k}->{score} )
			  if exists $hgene->{all_variants}->{$k}->{mother};
			push( @{ $class->{father} }, $hgene->{all_variants}->{$k}->{score} )
			  if exists $hgene->{all_variants}->{$k}->{father};
		}
		if (   scalar( @{ $class->{mother} } ) > 0
			&& scalar( @{ $class->{father} } ) == 0
			&& exists $hgene->{father}->{id} )
		{
		...
		elsif (scalar( @{ $class->{father} } ) > 0
			&& scalar( @{ $class->{mother} } ) == 0
			&& exists $hgene->{mother}->{id} )
		{
		...
		if (   $score_father + $score_mother > $score_biallelic
			&& $patient->getFamily->isTrio() )
		{
			$hgene->{max_score} =
			  $hgene->{score} + $score_father + $score_mother;
		}
		else {

			$hgene->{max_score} =
			  $hgene->{score} + $score_biallelic + $hgene->{penality};
		}
	}
		...
		sub {
			...
			foreach my $gene ( @{ $h->{genes} } ) {
				...
				unless ( exists $hgenes->{$gid} ) {
					...
					#my $chr =
					$hgenes->{$gid} = $gene;
					$hgenes->{$gid}->{score_father_mother} =
					  $hgenes->{$gid}->{score_mother} +
					  $hgenes->{$gid}->{score_father};

					$hgenes->{$gid}->{max_score} =
					  $hgenes->{$gid}->{score} +
					  ( $hgenes->{$gid}->{score_father_mother} >
						  $hgenes->{$gid}->{score_biallelic}
						? $hgenes->{$gid}->{score_father_mother}
						: $hgenes->{$gid}->{score_biallelic} );
					$hgenes->{$gid}->{max_score} -=
					  ( $hgenes->{$gid}->{denovo_rare} * 0.3 )
					  if $hgenes->{$gid}->{denovo_rare} > 2;
					$hgenes->{$gid}->{max_score} += 2
					  if $hgenes->{$gid}->{cnv_dup} > 1
					  or $hgenes->{$gid}->{cnv_del} > 1;
...
#CHANG LMDB POLYVIEWER : $chr->{$cname} = $project->getChromosome($cname)->lmdb_hash_variants("r") unless exists $dchr->{$cname};
		...
		if ($hg) {
			if ($hash_genes_panel) {
				...
				push( @$agenes,
					grep { $_->{mask} & $maskcoding } @{ $hg->{array} } );
			}
...
"<tr style='background-image: linear-gradient(to right, white, #d8eef9);'>"
			...
			$v->{id} = $vid;
			update_variant_editor::table_dejavu_live( $v, $patient->project,
				$patient, $g );
			my $t = time;
			...
			update_variant_editor::trio( undef, $v, $patient, $project )
			...
			foreach my $t (@headers) {
				if ( $t =~ 'igv' and $v->{value}->{type} ne 'substitution' ) {
					...
					if (   $v->{value}->{type} eq 'deletion'
						or $v->{value}->{type} eq 'large_deletion'
						or $v->{value}->{type} eq 'large_duplication' )
					{
						my $start2 = $start - 100;
					...
					$v->{html}->{$t} =~ s/$locus1/$locus2/
					  unless ( $v->{html}->{$t} =~ /$locus2/ );
				}
				if ( $t eq 'var_name' ) {
					if ( exists $v->{value}->{manta}->{is_imprecise}
						and $v->{value}->{manta}->{is_imprecise} == 1 )
					{
						$v->{html}->{$t} .=
qq{<br><span style='font-size:7px'><b><u>IS IMPRECISE</b></u></span>};
					}
					if ( exists $v->{value}->{cnv_details_genes}
						and scalar
						keys %{ $v->{value}->{cnv_details_genes} } > 1 )
					{
						my @lGenesNames;
						push( @lGenesNames,
"<table class='table table-striped table-condensed table-bordered table-hover table-mybordered'>"
						);
						push( @lGenesNames, "<tr>" );
						push( @lGenesNames,
"<td style='text-align:center;'><b>Gene Name</b></td>"
						);
						push( @lGenesNames,
"<td style='text-align:center;'><b>Nb Panel(s)</b></td>"
						);
						push( @lGenesNames,
"<td style='text-align:center;'><b>Description</b></td>"
						...
						push( @lGenesNames, "</tr>" );
						foreach my $g_id (
							keys %{ $v->{value}->{cnv_details_genes} } )
						{
							push( @lGenesNames, "<tr>" );
							my $cmd =
qq{"view_var_from_proj_gene_pat('$project_name', '$g_id', '$patient_name', '', '', '');"};
							my $disabled = '';
							...
							push( @lGenesNames,
"<td style='text-align:center;'><button onclick=$cmd $disabled>"
								  . $this_g->external_name()
								  . "</button></td>" );
							push( @lGenesNames,
						...
						$v->{html}->{$t} .=
qq{<br><br><button id="$id_info" type="button" class="btn btn-xs  btn-primary" style="background-color: #9796C4;font-size: 7px;font-family:  Verdana;color:white">Multi Genes !</button>};
						$v->{html}->{$t} .=
qq{<div data-dojo-type="dijit/Tooltip" data-dojo-props="connectId:'$id_info',position:['after']"><div style="width:450px;height:auto;text-align:center;">$genes_text</div></div>};
					}
				}
				if ( $t eq 'trio' ) {
					my $cnv_score;
					...
					my $vtype = '';
					$vtype =
'<span style="text-align:center;font-size:7px"><b><i>Large Deletion</b></i></span><br><br>'
					  if ( $v->{value}->{type} eq 'large_deletion' );
					$vtype =
'<span style="text-align:center;font-size:7px"><b><i>Large Duplication</b></i></span><br><br>'
					  if ( $v->{value}->{type} eq 'large_duplication' );
					$v->{html}->{$t} =~ s/<table/$vtype<table id="$id_info"/;
					$v->{html}->{$t} .=
qq{<div data-dojo-type="dijit/Tooltip" data-dojo-props="connectId:'$id_info',position:['after']"><div style="min-width:300px;width:auto;height:auto;"><center><b><u>Patient $patient_name</b></u><br><br>};

					if ( exists $v->{value}->{manta}->{is_imprecise}
						and $v->{value}->{manta}->{is_imprecise} == 1 )
					{
						$text_caller .=
						  qq{<br><br><b><u>IS IMPRECISE (MANTA)</b></u>};
					}

					$v->{html}->{$t} .=
qq{<b><u>Calling Method(s):</b></u><br> $text_caller </center></div></div>};
			...
			update_variant_editor::check_is_hgmd_dm_for_gene( $patient, $v,
				$g );    #TODO: if DM a faire
			update_variant_editor::check_is_clinvar_pathogenic_for_gene(
				$patient, $v, $g );    #TODO: if CLINVAR a faire
			$out .= $cgi->td( $style,
				update_variant_editor::table_validation( $patient, $v, $g ) );
			if (   $v->{value}->{type} eq 'large_deletion'
				or $v->{value}->{type} eq 'large_duplication' )
			{
				$out .= $cgi->td(
					$style,
					update_variant_editor::table_transcripts_cnv(
						$v,       $v->{genes}->{ $g->{id} },
						$g->{id}, \@header_transcripts_cnv
					)
				);
			}
			else {
				$out .= $cgi->td(
					$style,
					update_variant_editor::table_transcripts(
						$v->{genes}->{ $g->{id} },
						\@header_transcripts
					)
				);
...
sub constructChromosomeVectorsPolyDiagFork {
	...
	$filter_transmission->{xor_mother}      = 1
	  if $cgi->param('xor_mother')
	  or $cgi->param('xor');
	$filter_transmission->{xor_father} = 1
	  if $cgi->param('xor_father')
	  or $cgi->param('xor');

	#$filter_transmission->{xor}      = 1 if  $cgi->param('xor');
	if (   exists $filter_transmission->{xor_mother}
		&& exists $filter_transmission->{xor_father} )
	{
	...
	$pm->run_on_finish(
		sub {
			...
			push( @$list_variants, @{ $h->{list_variants} } )
			  if $h->{list_variants};
	...
	foreach my $chr ( @{ $project->getChromosomes } ) {
		...
		if ($panel) {
			...
			$hashVector->{ $chr->name } &=
			  $chr->getVectorScore($limit_ac);    #if $limit_ac ne "all";
			...
			$hashVector->{ $chr->name } &=
			  $chr->getVectorScore($limit_sample_dv_ho);
			...
			$hashVector->{ $chr->name } &= $chr->getVariantsVector();
			warn "\n 1 ==> " . $hashVector->{ $chr->name }->contains($testid)
			  if $debug;
			$hashVector->{ $chr->name } = $chr->getVectorScore($limit_ac)
			  ;    # if $limit_ac ne "gnomad_ac_all";;
			warn "\n 1 ==> " . $hashVector->{ $chr->name }->contains($testid)
			  if $debug;
  ...
  #warn $patient->countThisVariants( $patient->getVectorOrigin($chr)) if $debug;
		...
		warn "\n 1 ==> " . $hashVector->{ $chr->name }->contains($testid)
		  if $debug;
		warn "\n\n+" . $patient->getVectorOrigin($chr)->contains($testid)
		  if $debug;
		...
		warn "2"
		  if $debug && $hashVector->{ $chr->name }->contains($testid) && $debug;

		$statistics->{variations} +=
		  $patient->countThisVariants( $hashVector->{ $chr->name } );
		...
		if ($trio) {
			...
			if ( $patient->getFamily()->isDominant() ) {
				my $none = undef;
				if ( exists $filter_transmission->{xor_mother} ) {
					$vtr |= $patient->getFamily()
					  ->getVector_individual_mother( $chr, $patient );
					$none = 1;
				}
				if ( exists $filter_transmission->{xor_father} ) {
					$vtr |= $patient->getFamily()
					  ->getVector_individual_father( $chr, $patient );
					$none = 1;
				}
				...
				unless ($none) {
					$vtr |=
					  $patient->getFamily()->getVector_family_dominant($chr);
				}
...
#	$v1 -= $patient->getFamily()->getVectorRecessiveTransmission( $chr, $patient );
				...
				if ( $chr->name eq $gene_exception->getChromosome->name ) {
...
#  die($chr->name) if $debug && $hashVector->{ $chr->name }->contains(7710) && $debug;
		$hashVector->{ $chr->name } &= $hashVector_panel->{ $chr->name }
		  if exists $hashVector_panel->{ $chr->name };
		$statistics->{variations} +=
		  $patient->countThisVariants( $hashVector->{ $chr->name } );
		...
		if ( $limit_ratio > 0 ) {
			...
			if ( $limit_ratio1 < 100 ) {
				my $vector_ratio_name =
				  $patient->name . "_ratio_" . $limit_ratio1;
			...
			if ( $limit_ratio2 != $limit_ratio1 ) {
				...
				my $vector_ratio_name =
				  $patient->name . "_ratio_" . $limit_ratio2;
				$vector_ratio_name = $patient->name . "_ratio_all"
				  if $limit_ratio2 == -1;
		...
		if ( $in_this_run < 100 ) {
			...
			foreach my $id ( @{ to_array( $res->{vector}, $chr->name ) } ) {
				...
				my $vobj = $project->returnVariants($id)
				  ;    #$chr->cache_lmdb_variations->get_varid($id);
...
sub date_cache_bam {
	...
	( $date->{bam} ) =
	  utility::return_date_from_file( $patient->getBamFileName );
	return $date;
}
