...
$vector_clinical->{hgmd}=  $chr->getNewVector() ;
$vector_clinical->{dm}=  $chr->getNewVector();
...
foreach my $r (keys %$vector_clinical){
	warn  $r." ".$vector_clinical->{$r}->to_Hex();
...
$no3->put("transcripts_vector_annotations",$array);
