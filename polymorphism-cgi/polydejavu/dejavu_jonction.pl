...
$project_name = $buffer->getRandomProjectName('HG19_CNG') if not $project_name;
my $project = $buffer->newProjectCache(-name => $project_name);
$project->getChromosomes();

my $patient;
$patient = $project->getPatient($patient_name) if $patient_name;


my $table_id_my_var = "table_dv_".$project_name."_".$patient_name."_".$junction_vector_id_search.'_my_var';
$table_id_my_var = "table_dv_".$project_name."_".$patient_name."_".$junction_id_search.'_my_var' if ($junction_id_search and not $junction_vector_id_search);
...
$table_id = "table_dv".$project_name."_".$patient_name."_".$junction_id_search if ($junction_id_search and not $junction_vector_id_search);
my $html;
...
if ($junction_id_search) {
	my $junction = $project->_newJunction($junction_id_search);
	push(@lJunction, $junction);
	$chr_id = $junction->getChromosome->id();
}
else {
	($chr_id, $vector_id) = split('-', $junction_vector_id_search);
	my $chr = $project->getChromosome($chr_id);
	my $vector = $chr->getNewVector();
	$vector->Bit_On($vector_id);
	@lJunction = @{$chr->getListVarObjects($vector)};
}

#ICI ajout de la jucntion manuellement
...
foreach my $junction (@lJunction) {
	...
	if ($patient) {
		$list_sashimi_plot_files = $junction->getListSashimiPlotsPathFiles($patient);
		...
		foreach my $pos (sort keys %{$hdv->{$identity}}) {
			foreach my $prn (reverse sort keys %{$hdv->{$identity}->{$pos}}) {
				...
				foreach my $ptn (sort keys %{$hdv->{$identity}->{$pos}->{$prn}}) {
					...
					my $sashimi_pdf_tmp;
					if ($patient) {
						$sashimi_pdf_tmp = $project_tmp->getProjectPath.'/align/sashimi_plots/';
					...
					if ($patient_name and $prn eq $project_name and $ptn eq $patient_name) { $html_my_var .= $this_html; }
					else { $html .= $this_html; }
				}
				$project_tmp = undef;
				$buffer_tmp = undef;
