...
foreach my $patobj (@$listPatients)
{
	...
	#gather_strict_identicalCNV($patname);
	
	# on freeze la table correspondant à chaque patient individuelement
	#my $file_out = "/data-xfs/Manue/Test_SV/".$projectname."/newVersion/".$patname.".allCNV";
	#my $file_out = $variationsDir.$patname.".allCNV";
...
#my $file_dejavu_inthisproject = "/data-xfs/Manue/Test_SV/".$projectname."/newVersion/".$projectname."_dejavu.allCNV";
my $file_dejavu_inthisproject = $variationsDir.$projectname."_dejavu.allCNV";
my $file_dejavu = "/data-xfs/Manue/Test_SV/DejaVu/newVersion/".$projectname.".dejavu";
...
sub saveVariant()
{
	...
	print " Save : $compteur :  $patname $Caller $SVtype $SVchr $SVdeb $SVend \n";
