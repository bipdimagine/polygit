...
# recupere les options  (ici -project -patient) 
...
foreach my $patient (@$patients)
{
	...
	foreach my $chr (@$chrs)
	{
		...
		{
			$score =  $p->cnv_score($patient);
			warn $patient_name." ".$chr_name." ".$score;
			next if $score == -1;
			...
			if( ($v < -0.5) || ( $v > 0.5)) 	#deletion ou duplication
			{
				$start = $p->start;
				$end = $p->end;
				die if $end < $start;
				print $patient_name."\t".$chr_name."\t".$start."\t".$end."\t".$v."\n";
			}
