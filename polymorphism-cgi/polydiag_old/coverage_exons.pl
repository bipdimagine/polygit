...
use lib "$Bin/../GenBo/lib/obj-nodb/packages_old/";
#use Set::;
...
foreach my $tr (sort {$a->{gene_name} cmp $b->{gene_name}} values %$all_htranscripts) {
	my $tn = $tr->{vname};
	#print $cgi->start_table({class=>"coveragetable"});
		print $cgi->start_table({class=>"table table-striped table-bordered  table-condensed ",style=>"text-align: center;vertical-align:middle;font-size: 10px;font-family:  Verdana;"});
	print $cgi->start_Tr({class=>"info"});
	my $click3 = qq{load_graph_gene('$tn');};
	print $cgi->th({onClick=>$click3,scope=>"col", abbr=>"Configurations"}, $tr->{name});
	...
	#		warn $pn." ".$patient_red->{$tr->{id}}->{$patient->name}." ".scalar (keys %{$common_red->{$tr->{id} } } );
		...
		print $cgi->th({onClick=>$click2,scope=>"col"},$patient->name());
		push(@write_patients,$patient);
	}
	print $cgi->end_Tr();
	...
	foreach my $exon1 (sort{$a->{id} <=> $b->{id} }@{$tr->{exons}}){
		...
		if  (exists $common_red->{$tr->{id}}->{$exon1->{id}}){
			...
			print $cgi->td({class=>"",onClick=>$click4,scope=>"row", abbr=>"Model"}, $exon1->{vname});
		}
		foreach my $patient (@write_patients){
			...
			#my $text = $exon->{mean}."<small>(".$exon->{min}.")<br>[".$exon->{intspan}."]</small>";
			$jsid++;
			my $click = qq{load_graph_exon('$pn','$tn','$en');};
			my $check;
			...
			my ($r,$g,$b) = @{$exon->{color}};
			print $cgi->td({class=>"red",style=>"background-color:rgb($r,$g,$b);font-size:10px;",id=>"cov_$id"},$text);
#			if ($exon->{type} == 1){
