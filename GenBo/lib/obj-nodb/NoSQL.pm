...
NoSQL - NoSQL-ish overlay for an SQL database
...
=head1 SYNOPSIS
    ...
    my $store = NoSQL->connect( 'store.sqlite' );
...
NoSQL is a layer over DBI that presents a NoSQLish way to store and retrieve data. It does this by using a table called C<__Store__>. Once connected to a database, it will detect if this table is missing and create it if necessary
...
=head2 $store = NoSQL->connect( $path )

Returns a new NoSQL store connected to the SQLite database located at C<$path>
...
Return a L<NoSQL::Search> object for C<$model>, filtering on the optional C<$where>
