...
sub create_table {
	...
	$self->dbh($key1)->do("CREATE TABLE if not exists $table_name (_key VARCHAR(250),ho INTEGER, projects INTEGER, _value BLOB,start INTEGER,end INTEGER, variation_type VARCHAR(2), length INTEGER) ")  or die $DBI::errstr;;
...
sub prepare_cnv {
	...
	return $self->{prepare_cnv}->{$chr} if exists $self->{prepare_cnv}->{$chr};
		 my $table_name = $self->create_table($chr);
	$self->{prepare_cnv}->{$chr} = $self->dbh($chr)->prepare(qq{select $table_name.variation_type, $table_name.projects,$table_name.start,$table_name.end,$table_name._value from $table_name  where end >= ? and start<=?  and variation_type=? and (end - start) > ? and  (end - start)< ?  });
...
sub get_cnv {
	...
	confess();
	 ...
	 $l = abs($l*$len)-1;
	  my $min = $len - $l;
	   my $max = $len + $l;
	my $res = $self->dbh($chr)->selectall_arrayref(
			qq{select variation_type, projects,start,end,$table_name._value from $table_name  where end >= $start and start<=$end  and variation_type="$type"
				and (end - start) between $min and $max} );
	my $x;
	my$nb = 0;
