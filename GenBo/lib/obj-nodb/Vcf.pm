...
sub _read_column_names
{
    my ($self) = @_;
    my $line = $self->next_line();
    if ( !defined $line or substr($line,0,1) ne '#' ) { $self->throw("Broken VCF header, no column names? ".$self->{file}); }
...
sub Vcf4_0::parse_header_line
{
    ...
    while ($tmp ne '')
    {
        ...
        $attr_value = "TOTO"  unless $attr_value ;#trick for seqnext;
		        	
        	#next unless $attr_value;
        #		warn $attr_key." --> ".$attr_value;
        		#die();
        		die($tmp) unless defined $attr_value;
...
sub Vcf4_0::event_type
{
    ...
    if ( $len==$reflen )
    {
        ...
        for (my $i=0; $i<$len; $i++)
        {
            if ( substr($ref,$i,1) ne substr($allele,$i,1) ) { $mism++; $ht.=substr($allele,$i,1); $mism++; warn "+".$ht if $debug;$pos = $i unless $pos; }
        }
        if ( $mism==0 ) { $type='r'; $len=0; }
        else { $type='s'; $len=$mism; $allele=$ht; }
    }
    else
    {
