...
die("and the method ?") if $method ne "unifiedgenotyper" && $method ne "haplotypecaller_gvcf" && $method ne "haplotypecaller_vcf" && $method ne "haplotypecaller"&& $method ne "vcf" ;
...
my $pipeline = calling_steps->new(log_file=>$file, project=>$project,method_calling=>$method );
...
my @running_steps;
 ...
 @running_steps =("haplotypecaller" ) if $method eq "haplotypecaller_gvcf"; #plink
 @running_steps =("lifinder","genotype_gvcf","correct_vcf", "quality_check" ) if $method eq "haplotypecaller";
  @running_steps =("lifinder","genotype_gvcf","correct_vcf", "quality_check" ) if $method eq "vcf";
...
die() if ($choice ne "y"); 
if ($stdout) {
	print  $pipeline->bds_header()."\n";
foreach my $job (@{$pipeline->jobs_bds}){
	
	
	
	if (exists $job->{wait_exit}){
		print  $pipeline->bds_wait_and_exit(job=>$job);
		next;
		
	}
	elsif  (exists $job->{wait}){
		print  	$pipeline->bds_wait(job=>$job);
		next;
	}
#if ($job->{cmd} !~/ wait/){
		print  $pipeline->bds_system(job=>$job);
	#	next;
	#}
...
open(BDS,">test.bds");
print BDS $pipeline->bds_header()."\n";
foreach my $job (@{$pipeline->jobs_bds}){
	
	
	
	if (exists $job->{wait_exit}){
		print BDS $pipeline->bds_wait_and_exit(job=>$job);
		next;
		
	}
	elsif  (exists $job->{wait}){
		print BDS 	$pipeline->bds_wait(job=>$job);
		next;
	}
#if ($job->{cmd} !~/ wait/){
		print BDS $pipeline->bds_system(job=>$job);
	#	next;
	#}
...
if ($no_cluster){
	system("/bip-d/soft/distrib/bds/.bds/bds   test.bds");
}
else {
system("/bip-d/soft/distrib/bds/.bds/bds  -s cluster  test.bds");
}
##### METHODS ######
