...
sub cnv_dejavu_cache {
	...
	my $parquet_file = $self->project->buffer->config_path("dejavu","cnv").$self->project->name.".".$self->project->id.".parquet";
	my $ppn = $self->nproc;
	my $type = "cnv-dv-parquet";
	my $stepname = $projectName."@".$type;
	my $ok = $self->project->getCacheBitVectorDir()."/../logs/cnv-dv-parquet.ok";
	my $log_1c = $self->project->getCacheBitVectorDir()."/../logs/cnv-dv-parquet.1cnv.log";
	my $log_2c = $self->project->getCacheBitVectorDir()."/../logs/cnv-dv-parquet.2cnv.log";
	...
	$cmd .= " && perl $bin_dev/manue_cnv/cnv/2_cnv.pl -project=$projectName -fork=$ppn 1>$log_2c 2>>$log_2c";
	$cmd .= " && touch $ok";
