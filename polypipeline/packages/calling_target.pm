...
sub mutect2 {
	...
	while ( my @vals = $it->() ) {
		...
		my $cmd_mutect2 = qq{ $gatk Mutect2 --base-quality-score-threshold 20  -tumor $name -R $reference $gatk_region   $bam_file_string_hc   -O $mutect_out ;  $bcftools norm -f $reference -m -any $mutect_out -o $vcf_out  };
		system($cmd_mutect2);
		unlink $bed;
		$res->{file} = $vcf_out;
		$res->{nb}   = $nb;
