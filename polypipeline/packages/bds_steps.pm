...
"same fastq file present in two different patient : $name $file1 $file2"
		...
		my $fileout = $dirout . "/".$name."/" . $name . "-SIMPLIFIED_mergedIS.txt";
		my $prodout = $self->project->getVariationsDir("htlv1_calling")."/".$name."-SIMPLIFIED_mergedIS.txt";
...
"perl $bin_dev/htlv1.pl -project=$project_name  -patient=$name  -f1=$f1 -f2=$f2 $out_dir -ls=$ls -ln=$ln -fork=$ppn ";
		...
		if ( $self->unforce() && (( -e $fileout ) || (-e $prodout))) {
			$job_bds->skip();
		}
