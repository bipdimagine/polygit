...
method count_featureCounts  (Str :$filein! ){
	...
	my $cmd = "$featureCounts -T $ppn   -a $gtf --ignoreDup -o $fileout -p -t exon  ".join(" ",@bams)."&& $sed";
#	warn $cmd;
	...
	my $stepname = $project_name."@".$type;
		...
		my $cmd2 = "$featureCounts -T $ppn -a $gtf -f   -t exon  -O --ignoreDup -p  -o $fileout2  ".join(" ",@bams)." && $sed2";
