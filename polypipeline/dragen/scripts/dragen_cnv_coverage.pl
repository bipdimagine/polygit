...
my $cmd = qq{dragen -f -r $ref_dragen --output-directory $dir_pipeline --bam-input $bamin --output-file-prefix $patient_name --enable-map-align false   --vc-target-bed $capture_file --vc-target-bed-padding 150 --enable-cnv true --cnv-enable-self-normalization true };
if ($project->isGenome){
	my $cmd = qq{dragen -f -r $ref_dragen --output-directory $dir_pipeline --bam-input $bamin --output-file-prefix $patient_name --enable-map-align false --enable-cnv true --cnv-enable-self-normalization true };
}
...
move_gvcf($patient,$gvcf_pipeline);
my $target_pipeline  ="$dir_pipeline/".$patient_name.".target.counts.gz";
my $target_pipeline_gc  = "$dir_pipeline/".$patient_name.".target.counts.gc-corrected.gz";
move_target($target_pipeline,$target_pipeline_gc,$patient);
if($project->isGenome){
	my $cnv_file  = "$dir_pipeline/".$patient_name.".cnv.vcf.gz";
