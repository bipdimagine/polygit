...
# todo: Ajuster $max_dup et $min_map en fonction du type de projet ?
...
my $mapping_metrics_file = $dir_dragen_pipeline."/$patient_name.mapping_metrics.csv";
my $dir_stats_dragen = $patient->project->getAlignmentStatsDir("dragen-align");
my $mapping_metrics_file_2 =~ s/^$dir_dragen_pipeline/$dir_stats_dragen/;
$mapping_metrics_file = $$mapping_metrics_file_2 unless (-e $mapping_metrics_file);
confess("ERROR: No mapping metrics file found: '$mapping_metrics_file' or '$mapping_metrics_file_2'") unless (-e $mapping_metrics_file);
...
warn ("$patient_name: mapping = $mapping%, duplicates = $duplicate%");
...
if ($duplicate > $max_dup) {
	confess("Duplicate reads too high (> $max_dup) for patient '$patient_name': $duplicate\n");
#	die unless (prompt("Continue anyway ?  (y/n)  ", -yes_no));
}
if ($mapping < $min_map) {
