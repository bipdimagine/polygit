...
my $error;
if (keys %$ok) {
	print colored(['bright_green on_black']," SAMPLE OK IN SAMPLE SHEET: ".scalar(keys %$ok) )."\n";
}
if (keys %$ok_in_project) {
	print colored(['bright_green on_black']," SAMPLE OK IN PROJECT : ".scalar(keys %$ok_in_project) )."\n";
#	map {print $_."\n"} keys %$ok_in_project;
}
if (keys %patients) {
...
if (keys %$error_not_in_project) {
		print colored(['bright_red on_black']," SAMPLE IN SAMPLE SHEET NOT IN PROJECT : ".scalar(keys %$error_not_in_project) )."\n";
