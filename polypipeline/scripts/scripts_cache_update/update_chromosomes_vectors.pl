...
sub run_update {
	...
	push(@all_categories,'spliceAI_high');
	push(@all_categories,'spliceAI_medium');
	push(@all_categories,'predicted_splice_site');
	...
	foreach my $r (@$ranges){
		...
		for (my $i=$r->[0];$i<$r->[1]+1;$i++){
			...
			foreach my $g (@{$o->getGenes()}) { 		 
		 		my $h_spliceAI = $o->spliceAI_score($g);
				if ($h_spliceAI) {
					foreach my $cat (keys %$h_spliceAI) {
						my $value = $h_spliceAI->{$cat};
						next if ($value eq '-');
						if (defined($value) and $value >= $project->buffer->config->{spliceAI}->{high}) {
							$vector_categories_chromosomes->{spliceAI_high}->Bit_On($o->{index_lmdb});
							$vector_categories_chromosomes->{spliceAI_medium}->Bit_On($o->{index_lmdb});
							$vector_categories_chromosomes->{predicted_splice_site}->Bit_On($o->{index_lmdb});
						}
						elsif (defined($value) and $value >= $project->buffer->config->{spliceAI}->{medium}) {
							$vector_categories_chromosomes->{spliceAI_medium}->Bit_On($o->{index_lmdb});
							$vector_categories_chromosomes->{predicted_splice_site}->Bit_On($o->{index_lmdb});
						}
					}
