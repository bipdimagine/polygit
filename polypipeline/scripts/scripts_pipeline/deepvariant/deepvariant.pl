...
if ($project->isGenome) {
 ...
 $cmd = qq{singularity run  --bind /data-isilon:/data-isilon --bind /data-pure:/data-pure --bind /data-beegfs:/data-beegfs  /data-beegfs/software/sif/deepvariant1.8.0.sif /opt/deepvariant/bin/run_deepvariant   --model_type=WES --ref=$ref --reads=$bam --regions=$bed --output_vcf=$vcf_out -num_shards=$fork};
}
system($cmd);
warn $cmd;
