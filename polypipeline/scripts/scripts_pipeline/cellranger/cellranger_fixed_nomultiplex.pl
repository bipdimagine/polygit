...
foreach my $p (@{$patients_name}){
	...
	print CSV "[libraries]\n";
	#my $pobj=$pat[0]->{objet};
	my $fastq = $p->getSequencesDirectory();
	warn $fastq;
	print CSV "fastq_id,fastqs,feature_types\n";
	print CSV $name.",".$fastq.",Gene Expression\n";
	print CSV "\n";
#	print CSV "[samples]\n";
...
#		print CSV $p->{bc}."\n";
#	}
	print JOBS "cd $dir; cellranger multi --id=".$name." --csv=".$csv."\n";
}
...
if ($step eq "cp" or $step eq "all"){
	...
	foreach my $patient (@{$patients_name}) {
		...
		my $cmd= qq{rsync -rav  $dir/$pname $dirout };
		warn $cmd;
		system($cmd);
	}
}
