...
###############
if (grep(/demultiplex_hastags|count_multiplex|^all$/i, @steps)){
	...
	## Step 1 Demultiplex: Assign cells to samples
	## Step 1.1: Custom CMO reference
	# The CMO reference (or cmo-set) is similar to the feature_refence CSV used by Cell Ranger to describe Feature Barcodes.
	# The one difference is that the 'feature_type' column must have 'Multiplexing Capture' instead of 'Antibody Capture' as the Feature Barcode.
	# The feature_ref CSV can be modified running "sed 's/Antibody Capture/Multiplexing Capture/g' feature_ref.csv > cmo-set.csv"
	
	die("-cmo_ref option is mandatory with step 'demultiplex_hastags'") unless ($cmo_ref);
	...
	# [libraries] You only need two libraries here
	...
	foreach my $k (keys(%$hfamily)){
		...
		print CSV $poolName.",".$pool[0]->getSequencesDirectory().",Gene Expression\n";
		my $motif_cmo = $poolName =~ s/^(VDJ_)?/(ADT|CMO)_/r;
		my @cmo = grep {$_->name =~ /^$motif_cmo$/} @associated_patients;
		die("No cmo/adt 'CMO_".$poolName."' or 'ADT_".$poolName."' found in associated project '$associated_projectName'") unless (scalar @cmo);
#		my $cmo = $CMO[0];
		print CSV $cmo[0]->name.",".$cmo[0]->getSequencesDirectory().",Multiplexing Capture\n";
		print CSV "\n";
		print CSV "[samples]\n";
		print CSV "sample_id,cmo_ids\n";
		foreach my $p (@pat) {
			die("Can't find bc2 '".$p->{bc}."' in CMO reference\n", Dumper $cmo_set) unless (grep(/^$p->{bc}$/, @$cmo_set));
			print CSV $p->{name}.','.$p->{bc}."\n";
		}
		close (CSV);
		my $tmp_cmd = "cd $tmp; cellranger multi --id=$poolName --csv=$demultiplex_csv";
		$tmp_cmd = full_cmd($pool[0], $tmp_cmd, $dir, $tmp);
		print JOBS $tmp_cmd;
		$cmd .= $tmp_cmd;
	}
	close JOBS;
...
#	warn $cmd;
#	chomp $cmd;
	my $exit = system("cat $dir/jobs_count_multiplex.txt | run_cluster.pl -cpu=20") unless ($no_exec);
	exit($exit) if ($exit);
	...
	foreach my $patient (@$patients_name) {
		...
		my $file = $dir.'multiplex/*/'.$patient->name."/web_summary.html";
...
if (grep(/bamtofastq|^all$/i, @steps)) {
	...
	# $dir/multiplex/$poolName/outs/per_sample_outs/$sample/metrics_summary.csv 
	...
	foreach my $k (keys(%$hfamily)) {
		...
		while (my $line = <METRICS>) {
			...
			chomp $line;
			my @split = split('"', $line);
			$nb_reads->{$poolName} = $split[1] =~ s/,//rg;
		}
		close(METRICS);
#		warn $nb_reads->{$poolName};
		...
		my $rounded = int(($nb_reads->{$poolName} + 9_999_999) / 10_000_000) * 10_000_000; # Arrondi à la dizaine de million supérieure
#		warn $rounded;
		$rounded = 500_000_000 unless ($rounded >= 500_000_000);
		foreach my $pat (@pat) {
			my $pname = $pat->{name};
			...
			my $tmp_cmd = "cellranger bamtofastq --reads-per-fastq=$rounded $bam $tmp_fastq --nthreads=20";
			$cmd .= $tmp_cmd."\n";
			print JOBS $tmp_cmd."\n";
		}
#		warn $cmd if ($no_exec);
	...
	close(JOBS);
#	my $exit = system("cat $dir/jobs_bamtofastq.txt | run_cluster.pl -cpu=20") unless ($no_exec);
#	exit($exit) if ($exit);
	...
	foreach my $k (keys(%$hfamily)) {
		...
		foreach my $pat (@pat) {
			...
			my @gex_dir = grep {/^$poolName\_0_1_[A-Z0-9]{9}$/} readdir(DIR);	# $poolName\_1_1_[A-Z0-9]{9} pour les ADT
			warn ("No GEX directory found in $tmp_fastq") unless (scalar @gex_dir or not $no_exec);
			die ("No GEX directory found in $tmp_fastq") unless (scalar @gex_dir or $no_exec);
			die ("Expected 1 directory, found ".scalar @gex_dir) unless (scalar @gex_dir == 1 or $no_exec);
			my $gex_dir = $tmp_fastq.$gex_dir[0].'/';
			close(DIR);
...
#	warn Dumper $nb_cells;
	...
	warn $cmd if ($no_exec);
	chomp $cmd;
#	system("echo \"$cmd\" | run_cluster.pl -cpu=20") unless ($no_exec);
	my $exit = system ("cat $dir/jobs_count.txt | run_cluster.pl -cpu=20") unless ($no_exec);
	exit($exit) if ($exit);
}
