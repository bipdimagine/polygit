...
foreach my $chr (@{$project->getChromosomes}){
	...
	while (my $primer = $chr->next_primer){
		...
		foreach my $run (@$runs){
			my $run_id =  $run->id;
			push(@{$ps->{$run_id}},$primer);
		}
	}
	warn "end primer";
	my $res;
	my $nbp =0;
		foreach my $run_id (keys %$ps){
	...
	foreach my $run_id (keys %$ps){
		foreach my $hp (@{$control->{$run_id}}) {	
				...
				my $t1 =time;
				$nb++;
				warn "start $name :$nb/$nbp ".$chr->name;
				my $dir = $hp->{dir_depth};
				my $gb =  GenBoBinaryFile->new(name=>$name.".depth.lmdb",dir=>$dir,mode=>"r") ;
				#warn $primer->start." ".$primer->end;
				foreach my $primer (@{$ps->{$run_id}}){		
						my $array = $gb->getDepth($chr->name,$primer->start,$primer->end);
				...
				warn "end $name ".abs(time -$t1);
			}
			
		}
		warn "end ".$chr->name." ".abs(time-$t);
		$pm2->finish(0,$res);
	}
