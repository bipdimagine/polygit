...
foreach my $p (@{$project->getPatients}){
	...
	print CMD "$Bin/mito_remapping.pl -project=$project_name -patient=$patient_name -version=$version -fork=5 \n";
}
close (CMD);
my $dir1;
my $dir_v;
system("cat $dir_out/cmd1.list | run_cluster.pl -cpu=5");
exit(0);
#warn "***** END MAPPING *******";
#
#unlink "$dir_out/cmd1.list";
#warn " copy dude";
#my $dir1  = $project->getVariationsDir("dude");
#my $dir_v  = $project_v->getVariationsDir("dude");
...
#system("rsync -rav $dir1/ $dir_v/;rm $dir_v/*.cache*");
...
#my $cmd ="echo $Bin/../../scripts_cache/step/launch_chr_cache.pl -project=$project_name -fork=5 -chr=chrM -version=$version| run_cluster.pl -cpu=5";
#system($cmd);
#warn " compute dejavu";
...
sub update_version{
	...
	$dbh->{AutoCommit} = 0;
	my $query = qq{
		UPDATE PolyprojectNGS.project_release SET release_id=$version_id WHERE project_release_id=$project_id;
	};
	$dbh->do($query) ;
}
