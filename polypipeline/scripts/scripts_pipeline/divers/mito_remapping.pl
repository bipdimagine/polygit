...
my $bed = $patient->getCapture->gzFileName();
my $chr = $project->getChromosome("chrM");
my $intspan = $chr->getIntSpanCaptureForCalling(300);
 my $dir_out = $project->getCallingPipelineDir("mt_bam_".$patient_name);
...
system("$bazam -bam $bamo -L chrM:1-16572 | $bwa mem -t $fork -p -R '$readgroup' $ref_bwa  -  | samtools view -bSu - |  samtools sort -o $b2 -@ $fork");
system ("$samtools merge  $bam -h $b2 $b2 $b1  $b11 -@ $fork");
system ("$samtools index $bam -@ fork");
warn $bam;
}

sub create_gvcf {
	my $gvcf1 = $patient->getGvcfFile();
...
sub create_haplotypecaller {
	my $vcf1 = $patient->getVariationsFile("haplotypecaller4");
...
sub create_freebayes {
	my $vcf1 = $patient->getVariationsFile("freebayes");
...
sub create_samtools {
	my $vcf1 = $patient->getVariationsFile("samtools");
...
sub create_unifiedgenotyper {
	my $vcf1 = $patient->getVariationsFile("unifiedgenotyper");
