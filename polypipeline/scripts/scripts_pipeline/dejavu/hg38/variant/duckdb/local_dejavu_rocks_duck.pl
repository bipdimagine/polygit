...
open(CMD,">$filetmp");
 ...
 foreach my $chr (@chromosomes){
 		...
 		print  "/usr/bin/perl $Bin/dejavu_rocks.pl -chr=$chr -fork=5 -version=$version -file=$file -dir=$dir\n"; 
	system("/usr/bin/perl $Bin/dejavu_rocks.pl -chr=$chr -fork=5 -version=$version -file=$file -dir=$dir");
 	}
 }
exit(0);
 my $run;
