	...
	my $tloop = time;
	 while( my @tmp = $iter->() ){
		...
		foreach my $project_name (  @tmp) {
			...
			#next unless $project_name eq "NGS2021_4094";
		#	$dir_projects = "/data-isilon/DejaVu/HG19/";
			my $notodo = GenBoNoSql->new( dir => $dir_projects, mode => "r" );
			my $vh;
			eval {
			 $vh = $notodo->get( "$project_name", $chr, 1 );
			$notodo->close();
			};
			if ($@){
		...
		#	warn $project_name;
			foreach my $vid ( keys %$vh ) {
				my $debug;
				$debug = 1 if $vid  eq "12_80655799_G_A";
				next if $vid =~ /\|/;
...
#				my $nbhe = scalar(@he);
				...
				warn 	"$vid|$start|$end|$nbho|$string2|$type" if  $debug;
				print TMP "$vid|$start|$end|$nbho|$string2|$type\n";
				warn $string2 if $debug;
			}#end vid  
