...
my $fileout2 = $dir_out."/".$project_name.".count.exons.txt";
	...
	my $cmd = "$featureCounts -T $ppn   -a $gtf --ignoreDup -o $fileout -p -t exon  -s 0 ".join(" ",@bams)." && $sed";
	my $cmd2 = "$featureCounts -T $ppn -a $gtf -f   -t exon  -O --ignoreDup -p  -o $fileout2 -s 0 ".join(" ",@bams)." && $sed2";
