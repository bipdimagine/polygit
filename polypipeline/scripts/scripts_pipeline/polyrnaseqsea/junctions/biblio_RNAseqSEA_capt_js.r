...
rmdup<-function(I)
{
	...
	system(paste(sambambaPath, " markdup -r -t 3 ", bamsGene[I], " ", rmdupPath, substr(basename(bamsGene[I]), 0, (nchar(basename(bamsGene[I]))-4)), "_rmdup.bam", sep=""))
}

exJunBed<-function(J)	#	nom chromosome avec chr
{
	tmp = system(paste(sambambaPath, " view -t 3 ",bamsRMdups[J], " ", CHRbam, Chr, ":", Start-100, "_", End+100, " | cut -f3,4,6  | awk '{if ($3 ~ /N/){printf(\"%s\",$1); start=$2; end=$2; split($3,a,\"[NIMDSH]\"); 
...
JuncBam<-function(J)
{
	...
	{
		system(paste(sambambaPath, " view -t 3 -f 0x2 ",bamsRMdups[J], " | cut -f3,4,6  | awk '($3 ~ /N/)' > ", juncBamPath, gsub("_rmdup.bam", "", basename(bamsRMdups[J])), "_exons_junction_", Chr, ".bam", sep=""), intern=TRUE)
	}
}
