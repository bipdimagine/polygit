...
unlink $final_file if -e $final_file;
...
warn "$Bin/../construct_regions_freeze.pl -version=$version -project=$project_name -patient=$patient_name -window=$window_length -file=$file_freeze ";
...
warn "$Bin/join_gvcf.pl -version=$version -project=$project_name -patient=$patient_name -window=$window_length  -fork=$fork  -file=$file_freeze && rm  $dir_out_gvcf"."/* && rmdir $dir_out_gvcf";
...
chomp(@t);
confess('BIG BIG AND REBIG PROBLEM $final_gvcf') if $t[0] ne $patient->name();
