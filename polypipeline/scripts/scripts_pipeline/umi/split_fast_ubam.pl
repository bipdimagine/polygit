...
my $R2 = $dirin."/".$cp->{R3};#"/data-isilon/data/sequences/ILLUMINA/NOVASEQ/IMAGINE/run103.saved/2006N080737_LEBMI_S24_L001_R3_001.fastq.gz";
...
while ( ! eof($desc[0]) ) {
	...
	print $obam join("\t",($name,$line1,$seq[0],$qual[0],"RG:Z:$rg\tRX:Z:".$seq[2]))."\n";
	print $obam join("\t",($name,$line2,$seq[1],$qual[1],"RG:Z:$rg\tRX:Z:".$seq[2]))."\n";
	print $ofastq1 join("\n","@".$name,$seq[0],"+",$qual[0])."\n";
	print $ofastq2 join("\n","@".$name,$seq[1],"+",$qual[1])."\n";
