...
my $jobs_file = $project->getAlignmentDirName .'jobs_merge.txt';
open(my $fh, '>', $jobs_file) or confess("Can't open ".$jobs_file);
warn $jobs_file;
...
foreach my $pat (@$patients){
	...
	my $bam1_index = $bam1.'.bai' if ($bam1 =~ /\.bam$/);
	$bam1_index = $bam1.'.crai' if ($bam1 =~ /\.cram$/);
	...
	my $bam2_index = $bam2.'.bai' if ($bam2 =~ /\.bam$/);
	$bam2_index = $bam2.'.crai' if ($bam2 =~ /\.cram$/);
	
	my $merged_bam = $bam1 =~ s/(b|cr)am$/merged\.$1am/r || confess;
	my $merged_bam_index = $merged_bam.'.bai' if ($merged_bam =~ /\.bam$/);
	$merged_bam_index = $merged_bam.'.crai' if ($merged_bam =~ /\.cram$/);
	
	my $cmd->{$bam1}->{merge} = "$samtools merge -o $merged_bam $bam1 $bam2 -@ $cpu && samtools index $merged_bam ";
	confess ("'$merged_bam' already exits: $merged_bam") if (-e $merged_bam);
	
	# Efface ou renomme les bams partiels et mergés
	if ($rm_bam) {
		$cmd->{$bam1}->{mv} = "rm $bam1 $bam2 $bam1_index $bam2_index && mv $merged_bam $bam1 && mv $merged_bam_index $bam1_index ";
	}
	else {
		die("backup file '$bam1.sans_complement' already exists") if (-e $bam1.'.sans_complement');
		$cmd->{$bam1}->{mv} = "mv $bam1 $bam1.sans_complement && mv $bam1_index $bam1_index.sans_complement && mv $bam2 $bam2.complement && mv $bam2_index $bam2_index.complement && mv $merged_bam $bam1 && mv $merged_bam_index $bam1_index ";
	}

	# Créé des liens des fastq complémentaires dans le run du projet d'origine
	my @fastq_files = map {values %$_} @{$pat_dest->fastqFiles};
...
#		warn $fastq;
		my $link = $fastq;
		$link =~ s/\/\//\//;
		$link =~ m{^$dir_fastq_dest$pname\_S\d+_L(\d+)_R[12]_001.fastq.gz$} or next;
		my $L = sprintf("%.3d", $1 + 1);
#		warn $L;
		$link =~ s/^$dir_fastq_dest$pname\_S/$dir_fastq_complement$pname\_S/;
		$link =~ s/_L\d+_R([12])_001.fastq.gz$/_L$L\_R$1\_001.fastq.gz/;
#		warn $link;
		while (-e $link) {
			$L = sprintf("%.3d", $L + 1);
#			warn $L;
...
#			warn $link;
		...
		push (@ln_fastq , "ln -s $fastq $link");
	}
	$cmd->{$bam1}->{ln_fastq} = join( ' && ', @ln_fastq ) if (scalar @ln_fastq);
#	warn $cmd->{$bam1}->{ln_fastq};
	
	print {$fh} $cmd->{$bam1}->{merge} . ' && ' . $cmd->{$bam1}->{mv} . ' && ' . $cmd->{$bam1}->{ln_fastq} . "\n" if (exists $cmd->{$bam1}->{ln_fastq});
	print {$fh} $cmd->{$bam1}->{merge} . ' && ' . $cmd->{$bam1}->{mv} . "\n" unless (exists $cmd->{$bam1}->{ln_fastq});
}
close($fh);
system("cat $jobs_file | run_cluster.pl -cpu=$cpu") unless ($no_exec);
...
Optional arguments
	...
	-cpu <i>		number of cpu/threads to use
